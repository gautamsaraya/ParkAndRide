...
exports.createReservation = async (req, res) => {
  try {
    ...
    if (slot.status === 'maintenance') {
      return res.status(400).json({ message: 'Parking slot is under maintenance' });
    }

    // Check if the slot has any time restrictions that overlap with the requested period
    ...
    for (const restriction of slot.timeRestrictions || []) {
      ...
      if (
        ...
        return res.status(400).json({ 
          message: `Parking slot is not available: ${restriction.reason}`,
...
exports.cancelReservation = async (req, res) => {
  try {
    ...
    // Initialize refundAmount outside the if block
    let refundAmount = 0;
    let refundReason = '';
